...
function mapValues<T, U, K extends string>(
  ...
  const mappedEntries = entries.map<[K, U]>(([key, val]) => [
    key,
    mapFn(val, key),
  ]);
...
export const svelteNodeViewPlugin = (options: SvelteNodeviewPluginOptions) => {
  const nodeViews = mapValues(options.nodes, (ctor, name) =>
    createSvelteNodeView(ctor, { name })
  );
...
export function svelteSchemaNode(
  tag: string,
  ...
  return {
    ...
    parseDOM: [
      {
        ...
        getAttrs(dom: HTMLElement) {
          return mapValues(defaultData, (_, attr) =>
            JSON.parse(dom.getAttribute(`data-${attr}`))
          );
        },
      },
    ],
    ...
    toDOM(node) {
      const mappedEntries = Object.entries(node.attrs).map(([key, val]) => [
        `data-${key}`,
        JSON.stringify(val),
      ]);
